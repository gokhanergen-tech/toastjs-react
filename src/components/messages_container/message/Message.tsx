...
const Message = ({
  ...
  Component,
  position
}: withKey & { base: withKey; position: position }) => {
  ...
  const selectedType =
    types[hasError === null ? type : hasError === true ? 'error' : 'success']
  ...
  return (
    ...
    >
      ...
      <div className={styles.message_button}>
        <Button
          customClassName={button && button.className ? button.className : ''}
          disabled={removing}
          onClick={removeMessage}
          value={button && button.title ? button.title : 'OK'}
        ></Button>
      </div>
        ...
        ref={(instance) => {
          if (instance && autoCloseWithTimeout) {
            instance.style.setProperty(
              '--total_time',
              timeout +
              (animation.slideAnimation ? animation.animationDuration : 0) +
              'ms'
            )
          }
        }}
